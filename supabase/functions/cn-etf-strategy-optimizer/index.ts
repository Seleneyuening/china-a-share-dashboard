...
function dailyReviewKey(version: number) {
  ...
  return `ETF-${parts.year}-${parts.month}-${parts.day}-V${version}`;
}
...
Deno.serve(async () => {
  ...
  try {
    const [accountResult, strategyResult, tradesResult, snapshotsResult, runsResult] = await Promise.all([
      supabase.from("cn_portfolio_accounts").select("initial_capital,equity,strategy_version").eq("account_id", "etf").single(),
      supabase.from("cn_etf_strategy_versions").select("strategy_version,parameters,promoted_at").eq("status", "active").single(),
      supabase.from("cn_portfolio_trades").select("side,realized_pnl,strategy_version,occurred_at").eq("account_id", "etf").order("occurred_at", { ascending: true }).limit(1000),
      supabase.from("cn_portfolio_snapshots").select("captured_at,drawdown").eq("account_id", "etf").order("captured_at", { ascending: true }).limit(2000),
      supabase.from("cn_etf_engine_runs").select("started_at,status").order("started_at", { ascending: true }).limit(3000),
    ]);
    if (accountResult.error || !accountResult.data) throw new Error(accountResult.error?.message || "ETF account unavailable");
    if (strategyResult.error || !strategyResult.data) throw new Error(strategyResult.error?.message || "ETF strategy unavailable");

    const account = accountResult.data;
    const strategy = strategyResult.data;
    const currentVersion = number(strategy.strategy_version);
    const reviewKey = dailyReviewKey(currentVersion);
    const existingReview = await supabase.from("cn_etf_strategy_reviews").select("review_id,status,proposed_version").eq("review_key", reviewKey).maybeSingle();
    ...
    if (!enoughSample) {
      ...
      if (tradingDays >= 2 && versionAgeDays >= 2 && versionBuys === 0) {
        ...
        const rationale = `ETF V${currentVersion} 连续 ${tradingDays} 个交易日仍无买入样本，自动小幅放宽市场、评分和量能门槛。`;
        const promotion = await supabase.rpc("cn_promote_etf_strategy", { p_expected_current_version: currentVersion, p_parameters: nextParameters, p_evaluation: { ...metrics, versionBuys }, p_rationale: rationale });
        if (promotion.error) throw new Error(promotion.error.message);
        const nextVersion = number(promotion.data);
        await supabase.from("cn_etf_strategy_reviews").insert({ review_key: reviewKey, status: "promoted", current_version: currentVersion, proposed_version: nextVersion, metrics: { ...metrics, versionBuys }, rationale });
      ...
      const rationale = `ETF 账户继续收集样本：需要至少 10 个交易日、6 次完整卖出、当前版本 4 次完整卖出且运行 7 天。`;
      await supabase.from("cn_etf_strategy_reviews").insert({ review_key: reviewKey, status: "waiting_for_sample", current_version: currentVersion, metrics, rationale });
    ...
    if (!shouldDefend && !canExpand) {
      const rationale = `ETF V${currentVersion} 的收益与风险尚未形成明确改善方向，保持参数不变。`;
      await supabase.from("cn_etf_strategy_reviews").insert({ review_key: reviewKey, status: "no_change", current_version: currentVersion, metrics, rationale });
    ...
    const rationale = shouldDefend ? `ETF V${currentVersion} 的胜率或回撤触发防守升级。` : `ETF V${currentVersion} 在足够样本中保持正收益、较高胜率和低回撤，小幅扩大机会范围。`;
    const promotion = await supabase.rpc("cn_promote_etf_strategy", { p_expected_current_version: currentVersion, p_parameters: nextParameters, p_evaluation: metrics, p_rationale: rationale });
    if (promotion.error) throw new Error(promotion.error.message);
    const nextVersion = number(promotion.data);
    await supabase.from("cn_etf_strategy_reviews").insert({ review_key: reviewKey, status: "promoted", current_version: currentVersion, proposed_version: nextVersion, metrics, rationale });
