...
export type SupabasePortfolioState = { status: PortfolioStatus; mode: "real_quotes_paper_funds"; initialCapital: number; cash: number; equity: number; positions: LivePosition[]; trades: LiveTrade[]; snapshots: PortfolioSnapshot[]; strategyVersion: number; message: string; updatedAt: string; lastMarketAt?: string };
...
export function emptyPortfolioState(message = "Supabase A 股账户已建立，等待自主操盘引擎首次运行"): SupabasePortfolioState {
  ...
  return { status: "awaiting_engine", mode: "real_quotes_paper_funds", initialCapital: 1_000_000, cash: 1_000_000, equity: 1_000_000, positions: [], trades: [], snapshots: [{ occurredAt: now, equity: 1_000_000, cash: 1_000_000, cumulativeReturn: 0, drawdown: 0 }], strategyVersion: 1, message, updatedAt: now };
}

export async function loadPortfolioState(): Promise<SupabasePortfolioState> {
  try {
    const [accountResult, positionsResult, tradesResult, snapshotsResult] = await Promise.all([
    ...
    const error = accountResult.error || positionsResult.error || tradesResult.error || snapshotsResult.error;
