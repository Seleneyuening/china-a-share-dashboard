...
export function emptyPortfolioState(message = "Supabase A 股账户已建立，等待自主操盘引擎首次运行"): SupabasePortfolioState {
  ...
  return { status: "awaiting_engine", mode: "real_quotes_paper_funds", initialCapital: 1_000_000, cash: 1_000_000, equity: 1_000_000, positions: [], trades: [], snapshots: [{ occurredAt: now, equity: 1_000_000, cash: 1_000_000, cumulativeReturn: 0, drawdown: 0 }], strategyVersion: 1, evolution: { status: "not_reviewed", rationale: "自动进化已启用，等待首次周度复盘。", tradingDays: 0, closedTrades: 0, versionClosedTrades: 0, versionAgeDays: 0 }, message, updatedAt: now };
}
...
export async function loadPortfolioState(): Promise<SupabasePortfolioState> {
  try {
    const [accountResult, positionsResult, tradesResult, snapshotsResult, strategyResult, reviewResult] = await Promise.all([
      ...
      supabase.from("cn_strategy_reviews").select("status,rationale,metrics,reviewed_at").order("reviewed_at", { ascending: false }).limit(1).maybeSingle(),
    ]);
